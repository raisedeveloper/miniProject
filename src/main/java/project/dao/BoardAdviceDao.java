...
//			while (rs.next()) {
//				board3 = new Board3(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), 
...
//	// query 값은 검색어
//	public List<Board3> getBoardList(String field, String query, int num, int offset) {
//		Connection conn = getConnection();
//		String sql = "SELECT b.*, u.uname FROM board b"
//					+ "	JOIN users u ON b.uid=u.uid"
...
//					+ "	ORDER BY bid DESC "
//					+ "	LIMIT ? OFFSET ?";
//		List<Board3> list = new ArrayList<Board3>();
//		try {
//			PreparedStatement pstmt = conn.prepareStatement(sql);
...
//			ResultSet rs = pstmt.executeQuery();
//			while (rs.next()) {
//				Board3 board3 = new Board3(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), 
//						LocalDateTime.parse(rs.getString(5).replace(" ", "T")),
//						rs.getInt(6), rs.getInt(7), rs.getInt(8), rs.getString(9));
//				list.add(board3);
//			}
...
//		try {
//			PreparedStatement pstmt = conn.prepareStatement(sql);
//			pstmt.setString(1, board3.getTitle());
//			pstmt.setString(2, board3.getContent());
//			pstmt.setString(3, board3.getUid());
//			
//			pstmt.executeUpdate();
